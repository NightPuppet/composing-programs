...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # Initialize sum_of_dices and pig_out
    sum_of_dices = 0
    pig_out = False
    ...
    for rolls in range(num_rolls):
        ...
        # Check if rolled a 1 (Pig out)
        if current_roll == 1:
            pig_out = True

        # Add current_roll to sum_of_dices
        sum_of_dices += current_roll

    # Return 1 if pig_out, otherwise return sum_of_dices
    return 1 if pig_out else sum_of_dices
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    # If num_rolls is 0, apply Free bacon rule
    if num_rolls == 0:
        turn_score = 1 + max(opponent_score // 10, opponent_score % 10)
    # Otherwise, roll the dice and calculate the turn score
    else:
...
def select_dice(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Check if the sum of score and opponent_score is a multiple of 7
    is_hog_wild = (score + opponent_score) % 7 == 0

    # Return four_sided dice if Hog wild, otherwise return six_sided dice
    return four_sided if is_hog_wild else six_sided
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    while player1_score < goal and player0_score < goal:
        if who == 0:
            # Player 0's turn
            num_rolls = strategy0(player0_score, player1_score)
            player0_score += take_turn(num_rolls, player1_score, dice=select_dice(player0_score, player1_score))
            who = 1
        else:
            # Player 1's turn
            num_rolls = strategy1(player1_score, player0_score)
            player1_score += take_turn(num_rolls, player0_score, dice=select_dice(player1_score, player0_score))
            who = 0

        # Swap scores if one player's score is double the other's score
        if player0_score == player1_score * 2 or player1_score == player0_score * 2:
            player0_score, player1_score = player1_score, player0_score
...
def make_averaged(fn, num_samples=100000):
    ...
    def averaged_func(*args):
        total = 0
        # Calculate the sum of the function's results for num_samples times
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    for num_rolls in range(1, 11):
        ...
        print(f"{num_rolls} dice scores {avg_score} on average")
